...
$homepageBannerSlides = array(
    ...
    array(
        ...
        'source' => 'stravita.by',
        'image' => '/img/stravita-banner-photo.png',
        'image_alt' => 'Семейная пара на фоне летнего поля для баннера программы накопительного страхования.',
        'href' => '/nakopitelnaya-pensiya.php',
        'position' => 'center center',
        'image_fit' => 'cover',
        'media_ratio' => '3 / 1',
        'media_background' => '#fbfaf6',
        'overlay_variant' => 'stravita',
        'overlay_headline_lines' => array('ПЛАНИРУЙТЕ', 'БУДУЩЕЕ', 'УЖЕ СЕЙЧАС'),
        'overlay_badge_lines' => array('Накопительная пенсия', 'с участием государства'),
        'is_external' => false,
    ),
    array(
...
<head>
    ...
    <style>
        ...
        .homepage-banner__media--stravita {
            background: linear-gradient(90deg, rgba(251, 250, 246, 0.98) 0%, rgba(251, 250, 246, 0.92) 28%, rgba(251, 250, 246, 0.2) 56%, rgba(251, 250, 246, 0) 100%);
        }

        .homepage-banner__media--stravita::before {
            content: "";
            position: absolute;
            inset: 0;
            z-index: 1;
            background:
                linear-gradient(90deg, rgba(251, 250, 246, 1) 0%, rgba(251, 250, 246, 0.96) 26%, rgba(251, 250, 246, 0.36) 54%, rgba(251, 250, 246, 0) 76%),
        ...
        .homepage-banner__media--stravita::after {
            background: linear-gradient(180deg, rgba(18, 43, 34, 0) 72%, rgba(18, 43, 34, 0.06) 100%);
        }
        ...
        .homepage-banner__stravita-overlay {
            position: absolute;
            inset: 0;
            z-index: 2;
            display: flex;
        ...
        .homepage-banner__stravita-copy {
            display: flex;
            flex-direction: column;
            align-items: flex-start;
            gap: 12px;
        ...
        .homepage-banner__stravita-line {
            display: block;
            color: #1f5fb5;
        ...
        .homepage-banner__stravita-badge {
            display: inline-flex;
            flex-direction: column;
            align-items: flex-start;
            gap: 4px;
            ...
            background: #9ad312;
            box-shadow: 0 8px 20px rgba(112, 150, 16, 0.18);
        }

        .homepage-banner__stravita-badge-line {
            display: block;
            color: #fff;
...
<?php
    ...
    $slideImageFit = trim((string) ($slide['image_fit'] ?? ''));
    $slideMediaRatio = trim((string) ($slide['media_ratio'] ?? ''));
    $slideMediaBackground = trim((string) ($slide['media_background'] ?? ''));
    $slideOverlayVariant = trim((string) ($slide['overlay_variant'] ?? ''));
    $slideOverlayHeadlineLines = isset($slide['overlay_headline_lines']) && is_array($slide['overlay_headline_lines'])
        ? $slide['overlay_headline_lines']
        : array();
    $slideOverlayBadgeLines = isset($slide['overlay_badge_lines']) && is_array($slide['overlay_badge_lines'])
        ? $slide['overlay_badge_lines']
        : array();
    $slideStyleParts = array();
    if ($slidePosition !== '') {
        $slideStyleParts[] = 'object-position: ' . e($slidePosition);
    }
    if ($slideImageFit !== '') {
        $slideStyleParts[] = 'object-fit: ' . e($slideImageFit);
    }
    $slideStyle = !empty($slideStyleParts)
        ? implode('; ', $slideStyleParts) . ';'
        : '';
    $slideMediaStyleParts = array();
    if ($slideMediaRatio !== '') {
        $slideMediaStyleParts[] = 'aspect-ratio: ' . e($slideMediaRatio);
    }
    if ($slideMediaBackground !== '') {
        $slideMediaStyleParts[] = 'background: ' . e($slideMediaBackground);
    }
    $slideMediaStyle = !empty($slideMediaStyleParts)
        ? implode('; ', $slideMediaStyleParts) . ';'
        : '';
    ...
    <div class="homepage-banner__card<?php echo $slideVariant === 'hero' ? ' homepage-banner__card--hero' : ''; ?>">
        <div class="homepage-banner__media-wrap">
            <?php if ($slideVariant === 'hero'): ?>
                <div class="homepage-banner__media homepage-banner__media--hero"<?php echo $slideMediaStyle !== '' ? ' style="' . $slideMediaStyle . '"' : ''; ?>>
                <img
            ...
            >
                <div class="homepage-banner__media<?php echo $slideOverlayVariant === 'stravita' ? ' homepage-banner__media--stravita' : ''; ?>"<?php echo $slideMediaStyle !== '' ? ' style="' . $slideMediaStyle . '"' : ''; ?>>
                    <img
                        class="homepage-banner__image"
                        src="<?php echo e($slideImage); ?>"
                        alt="<?php echo e($slide['image_alt'] ?? ''); ?>"
                        ...
                        <?php echo $index === 0 ? 'fetchpriority="high"' : ''; ?>
                    >
                    <?php if ($slideOverlayVariant === 'stravita'): ?>
                    <div class="homepage-banner__stravita-overlay" aria-hidden="true">
                        <div class="homepage-banner__stravita-copy">
                            <?php foreach ($slideOverlayHeadlineLines as $slideOverlayHeadlineLine): ?>
                            <span class="homepage-banner__stravita-line"><?php echo e($slideOverlayHeadlineLine); ?></span>
                            <?php endforeach; ?>
                            <?php if ($slideOverlayBadgeLines !== array()): ?>
                            <span class="homepage-banner__stravita-badge">
                                <?php foreach ($slideOverlayBadgeLines as $slideOverlayBadgeLine): ?>
                                <span class="homepage-banner__stravita-badge-line"><?php echo e($slideOverlayBadgeLine); ?></span>
                                <?php endforeach; ?>
                            </span>
                            <?php endif; ?>
                        </div>
