...
function hasNewsSeoTitleCollision($conn, $title, $date, $id)
{
    ...
    $stmt->bind_param('ssi', $normalizedTitle, $normalizedDate, $id);
    $stmt->execute();
    $result = $stmt->get_result();
    $hasCollision = $result instanceof mysqli_result && $result->num_rows > 0;
    $stmt->close();
...
$hasSeoTitleCollision = hasNewsSeoTitleCollision($conn, $title, $date, $newsId);
$seoCollisionLabel = $hasSeoTitleCollision ? ' №' . $newsId : '';
$seoDateLabel = $formattedNumericDate !== '' ? $formattedNumericDate : 'без даты';
$seoTypeLabel = $type === 'новость' ? 'Новость' : 'Мероприятие';
$seoTitleMeta = $title . ' — ' . $seoDateLabel . $seoCollisionLabel . ' | ТЦСОН Гомель';
$seoDescriptionSource = trim(preg_replace('/\s+/u', ' ', strip_tags($description)));
$seoDescriptionSuffix = ' ' . $seoTypeLabel . ' от ' . $seoDateLabel
    . ($hasSeoTitleCollision ? ', публикация №' . $newsId : '')
    . '.';
$seoDescriptionMaxLength = 160;
$seoDescriptionSourceMaxLength = max(
    24,
    $seoDescriptionMaxLength - mb_strlen($seoDescriptionSuffix, 'UTF-8')
);

if ($seoDescriptionSource === '') {
    $seoDescriptionSource = 'Материал ТЦСОН Железнодорожного района г. Гомеля.';
}
if (mb_strlen($seoDescriptionSource, 'UTF-8') > $seoDescriptionSourceMaxLength) {
    $seoDescriptionSource = rtrim(
        mb_substr($seoDescriptionSource, 0, $seoDescriptionSourceMaxLength - 3, 'UTF-8')
    ) . '...';
}
$seoDescriptionMeta = $seoDescriptionSource . $seoDescriptionSuffix;
