...
function getManagedStaticPublicPages()
{
    return array(
        ...
        '/domestic-violence-prevention.php' => 'Профилактика домашнего насилия',
        '/gossocpodderzhka-invalidov.php' => 'Льготы, права и гарантии инвалидов',
        '/informatsiya.php' => 'Информация',
        '/easy-language.php' => 'Ясный язык - ясно и просто',
        '/questions-answers.php' => 'Вопрос-ответ',
        '/direct-phone-line.php' => 'Прямая телефонная линия',
        '/personal-reception.php' => 'График личного приема',
        '/electronic-appeals.php' => 'Электронные обращения граждан и юрлиц',
        '/about.php' => 'О центре',
        '/structure.php' => 'Структура учреждения',
        '/procedures.php' => 'Административные процедуры',
...
function getManagedStaticPublicPageDefaultVisibility()
{
    return array(
        '/procedures.php' => false,
    );
}
...
function fetchPublicPageVisibilityState($conn, $pageKey, $pageTitle = '')
{
    ...
    $defaultVisibility = getManagedStaticPublicPageDefaultVisibility();
    $isPublished = is_array($row)
        ? (int) ($row['is_published'] ?? 1) === 1
        : (bool) ($defaultVisibility[$normalizedKey] ?? true);
